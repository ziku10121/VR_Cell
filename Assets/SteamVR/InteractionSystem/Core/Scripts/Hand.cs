...
namespace Valve.VR.InteractionSystem
{
    ...
    {
        public static GameObject CurrentobjectToAttach;
        // The flags used to determine how an object is attached to the hand.
        [Flags]
        public enum AttachmentFlags
        {
            SnapOnAttach = 1 << 0, // The object should snap to the position of the specified attachment point on the hand.該對象與手上指定附著點對齊
            DetachOthers = 1 << 1, // Other objects attached to this hand will be detached.附著在此手上的其他物體將被分離
            DetachFromOtherHand = 1 << 2, // This object will be detached from the other hand.
            ParentToHand = 1 << 3, // The object will be parented to the hand.該對象將成為手子物件
        ...
        //-------------------------------------------------
        // Attach a GameObject to this GameObject               //將物件附加到此物件
        //
        // objectToAttach - The GameObject to attach            //objectToAttach附上物件
        // flags - The flags to use for attaching the object    //flags用於附加標誌屬性
        // attachmentPoint - Name of the GameObject in the hierarchy of this Hand which should act as the attachment point for this GameObject  //attachmentPoint此Hand層次結構中物件的名稱，應作為物件的point
        //-------------------------------------------------
        public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            CurrentobjectToAttach = objectToAttach;
            Debug.Log("抓取物件名稱:"+objectToAttach);
            //objectToAttach.gameObject.SetActive(false);
        ...
        public GrabTypes GetBestGrabbingType(GrabTypes preferred, bool forcePreference = false)
        {
            ...
            {
                // (grabPinchAction.GetState(handType))
                //    return GrabTypes.Pinch;
